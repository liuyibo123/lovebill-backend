 ...
 *
 * @author zhengzhiyuan
 * @info 孙振取自开源项目https://github.com/zhengzhiyuan/redisUtil
 * @since May 20, 2016
 */
public class ShardedRedisUtil {
    ...
    private void initialShardedPool() {
        // 操作超时时间,默认2秒
        int timeout = NumberUtils.toInt(SharedRedisConfig.getConfigProperty("master.redis.timeout"), 2000);
        // jedis池最大连接数总数，默认8
        int maxTotal = NumberUtils.toInt(SharedRedisConfig.getConfigProperty("master.redis.max_active"), 8);
        // jedis池最大空闲连接数，默认8
        int maxIdle = NumberUtils.toInt(SharedRedisConfig.getConfigProperty("master.redis.max_idle"), 8);
        // jedis池最少空闲连接数
        int minIdle = 0;
        // jedis池没有对象返回时，最大等待时间单位为毫秒
        long maxWaitMillis = NumberUtils.toLong(SharedRedisConfig.getConfigProperty("master.redis.max_wait"), -1);
        ...
        // 取得redis的url
        String redisUrls = SharedRedisConfig.getConfigProperty("master.redis.url");
        // 取得redis的密码
        String password = SharedRedisConfig.getConfigProperty("master.redis.password");
