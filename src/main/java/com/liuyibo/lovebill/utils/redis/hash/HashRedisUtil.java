...
public class HashRedisUtil {
    ...
    private void initPool() {
        ...
        // 操作超时时间,默认2秒
        int timeout = NumberUtils.toInt(HashRedisConfig.getConfigProperty("master.redis.timeout"), 2000);
        // jedis池最大连接数总数，默认8
        int maxTotal = NumberUtils.toInt(HashRedisConfig.getConfigProperty("master.redis.max_active"), 8);
        // jedis池最大空闲连接数，默认8
        int maxIdle = NumberUtils.toInt(HashRedisConfig.getConfigProperty("master.redis.max_idle"), 8);
        // jedis池最少空闲连接数
        int minIdle = 0;
        // jedis池没有对象返回时，最大等待时间单位为毫秒
        long maxWaitMillis = NumberUtils.toLong(HashRedisConfig.getConfigProperty("master.redis.max_wait"), -1);
